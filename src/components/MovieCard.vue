<template>
    <div class="movie-card">
        <div class="movie-image" :style="{'background-image': 'url(' + generateImageUrl(movie.poster_path) + ')'}" @click="showDetail()">
        ...
        <div class="movie-action">
            <ButtonAddToList type="movie" :movie="movie" :short="true" @removeFromList="removeFromList"/>
        </div>
    ...
    methods: {
        ...
        removeFromList() {
            if(this.$route.name == 'MyList') {
                this.$emit('removeFromList', this.movie)
            }
        }
    }
}
